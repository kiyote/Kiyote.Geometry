...
internal sealed class Rasterizer : IRasterizer {
	...
	void IRasterizer.Rasterize(
		...
		if( delta_y == 1 ) { // Horizontal line
			...
			for( int i = sl[0]; i <= sl[1]; i++ ) {
				callback.RasterizerCallback( i, small_y );
			}
			return;
		}
		...
		/* Step 3: go through all the lines in this polygon and build min/max x array. */
		for( int i = 0; i < polygon.Count; i++ ) {
			ind = ( i + 1 ) % polygon.Count;              //last line will link last vertex with the first (index num-1 to 0)
			if( polygon[ind].Y - polygon[i].Y != 0 ) {
				//initializing current line data (see tutorial on line rasterization for details)
				dx = polygon[ind].X - polygon[i].X;
				dy = polygon[ind].Y - polygon[i].Y;
				if( dx >= 0 ) {
					incXH = incXL = 1;
				} else {
					dx = -dx;
				...
				xc = polygon[i].X;
				yc = polygon[i].Y;       //initial current x/y values
				for( int j = 0; j <= longD; j++ ) { //step through the current line and remember min/max values at each y
					ind = yc - small_y;
					if( xc < sl[( ind * 2 ) + 0] ) {
						sl[( ind * 2 ) + 0] = xc;    //initial contains INT_MAX so any value is less
		...
		for( int i = 0; i < delta_y; i++ ) {
			for( int j = sl[( i * 2 ) + 0]; j <= sl[( i * 2 ) + 1]; j++ ) {
				callback.RasterizerCallback( j, i + small_y );
			}
		}
	}
	...
	void IRasterizer.Rasterize(
		...
		int error = dx + dy;
		while( true ) {
			callback.RasterizerCallback( x0, y0 );
			if( x0 == x1
				&& y0 == y1
			) {
				break;
