...
public sealed class VoronoiFactoryVisualizer {
	...
	private void VisualizeFixed() {
		int width = _bounds.Width - 200;
		int height = _bounds.Height - 200;
		...
		IVoronoi voronoi = _voronoiFactory.Create( delaunator, width, height );
		...
		image.SaveAsPng( Path.Combine( _outputFolder, "VoronoiFactory_Fixed.png" ) );
	...
	private void VisualizeRandom() {
		...
		image.SaveAsPng( Path.Combine( _outputFolder, "VoronoiFactory_Random.png" ) );
	}
}
